...
public class FXMLController {
    ...
    void doRun(ActionEvent event) {
    	...
    	try {
			...
			List<PowerOutages> worstCase = model.getSoluzione(maxY, maxH, selectedNerc);

			txtResult.clear();
			txtResult.appendText("Tot people affected: " + model.calcolaCA(worstCase) + "\n");
			txtResult.appendText("Tot hours of outage: " + model.calcolaOre(worstCase)/60 + "\n");

			for (PowerOutages ee : worstCase) {
				txtResult.appendText(String.format("%d %s %s %d %d", ee.getAnno(), ee.getInizio(), ee.getFine(),
						ee.getDurata()/60, ee.getCustomerAffected()));
    ...
    public void setModel(Model model) {
    	this.model = model;
    	List<Nerc> listaCombo = model.getNercList();
		cmbNerc.getItems().addAll(listaCombo);
    }
}
